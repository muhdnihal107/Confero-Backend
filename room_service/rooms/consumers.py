...
class RoomConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        user_email = getattr(user, 'email', None)
        if user_email:
            redis_client.sadd(f"room:{self.room_id}:participants", user_email)
            await self.channel_layer.group_send(
                self.room_group_name,
                {
                    'type': 'user_joined',
    ...
    async def disconnect(self, close_code):
        user_email = getattr(self.scope['user'], 'email', None)
        if user_email:
            redis_client.srem(f"room:{self.room_id}:participants", user_email)
        await self.channel_layer.group_discard(self.room_group_name, self.channel_name)
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if message_type == 'chat_message':
                ...
                await self.handle_webrtc_signal(data, message_type)
        except json.JSONDecodeError as e:
            await self.send(text_data=json.dumps({'error': 'Invalid message format'}))
    ...
    async def handle_webrtc_signal(self, data, signal_type):
        ...
        if not signal_data or not target:
            return
    ...
    async def webrtc_offer(self, event):
        if event.get('target') == getattr(self.scope['user'], 'email', None):
            await self.send(text_data=json.dumps({
                'type': 'webrtc_offer',
                'data': event['data'],
                'sender': event['sender'],
